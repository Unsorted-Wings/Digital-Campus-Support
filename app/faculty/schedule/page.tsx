...
export default function SchedulePage() {
  ...
  const courseOptions = Array.from(
    new Map(
      subjectData.map((s) => [
        s.courseId + "_" + s.batchId,
        {
          courseId: s.courseId,
          batchId: s.batchId,
          label: `${s.courseName} - ${s.batchName}`,
        },
      ])
    ).values()
  );
  ...
  const filteredSubjects = subjectData.filter(
    (s) =>
      ...
      s.batchId ===
        courseOptions.find((c) => c.courseId === selectedCourseId)?.batchId
  );
      ...
      const fetchSubjects = async () => {
        try {
          const response = await fetch(
            `/api/subjects/viewSubject/viewCourseWiseSubjects?teacherId=${user.uid}`
          );
          if (!response.ok) {
  ...
  const handleAddEvent = () => {
    ...
    const dataToSend = {
      ...
      description: title + " " + selectedSubjectName,
    };
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) => {
                  ...
                  setNewEvent({ ...newEvent, subject: "" }); // reset subject
                }}
              >
                ...
                <SelectContent>
                  {courseOptions.map((course) => (
                    <SelectItem
                      key={`${course.courseId}_${course.batchId}`}
