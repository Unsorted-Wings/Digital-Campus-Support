...
export async function GET(req: NextRequest) {
  try {
    ...
    const id = url.searchParams.get("id"); // semesterSubjectId

    if (!id) return NextResponse.json({ error: "Missing semesterSubject ID" }, { status: 400 });

    const doc = await admin.firestore().collection("semesterSubjects").doc(id).get();
